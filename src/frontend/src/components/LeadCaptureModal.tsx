...
import { type LeadData, useLeadCapture } from "../context/LeadCaptureContext";
        ...
        {!submitted && (
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {fields.map(
                ({ key, label, placeholder, type, icon, autoComplete }) => (
                  <div
                    key={key}
                    className={
                      key === "streetAddress" || key === "email"
                        ? "sm:col-span-2"
                        : ""
                    }
                  >
                    <Label
                      htmlFor={`lead-${key}`}
                      className="text-sm font-medium text-foreground mb-1.5 flex items-center gap-1.5"
                    >
            ...
            <p className="text-xs text-muted-foreground text-center leading-relaxed">
              By submitting, you agree to receive information about loan offers
              from our partners. Your information is used solely to personalize
              your experience. We do not sell your data.
            </p>
          </form>
        )}
